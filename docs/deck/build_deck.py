...
for i, (v, l, c) in enumerate([
        ("14 / 14", "prose fields reproduced\ncharacter for character", GREEN),
...
note(s, "The last number is deliberate. Three quarters of their format cannot be "
        "reached from a 6-column row at all — that is an argument from evidence.")
...
# --- 8. THE FIVE DESCRIPTIONS (optional wireframe slot) ----------------------
s = S[7]
heading(s, "One product, written five ways — to the character")
b, tf = txbox(s, X0, 1.44, W, 0.42)
rich(tf, [("Their guide: ", True, NAVY),
          ("“getting these formats right is most of the task.” Each line below is "
           "generated by formula and matches Unilog's labelled row exactly.",
           False, MUTED)], size=10, space_after=0, first=True, line=1.2)

rows = [
    ("INVOICE_DESC", "≤ 40, CAPS", "DISHWASHER LEG 5 SST 120V 15A 50-1/4IN", "38"),
    ("MOBILE_DESC", "60 – 80", "Rheem Manufacturing FRIGIDAIRE, Dishwasher, Professional Series, PDSH4816AF", "75"),
    ("SHORT_DESC", "title case", "FRIGIDAIRE® Professional Series PDSH4816AF Dishwasher With CleanBoost™, Leg Mounting, 5-Wash Cycle, Stainless Steel", "115"),
    ("RETAIL_DESC", "no brand/MPN", "Professional Series Dishwasher, Leg Mounting, 5-Wash Cycle, Stainless Steel", "75"),
]
y = 1.95
for name, rule, text, n in rows:
    card(s, X0, y, W, 0.66, fill=CARD)
    b, tf = txbox(s, X0 + 0.16, y + 0.10, 1.5, 0.45)
    para(tf, name, size=8.5, bold=True, color=NAVY, space_after=1, first=True)
    para(tf, rule, size=7.5, color=MUTED, space_after=0)
    b, tf = txbox(s, X0 + 1.74, y + 0.13, 6.5, 0.42)
    para(tf, text, size=8.5, color=INK, space_after=0, first=True, line=1.12)
    chip(s, X0 + 8.42, y + 0.19, 0.5, 0.28, n, fill=GREEN, size=8.5)
    y += 0.74

b, tf = txbox(s, X0, 4.92, W, 0.42)
rich(tf, [("Over-length copy drops whole tokens, never characters. ", True, AMBER),
          ("Cutting at 40 gives “…PDSH4816AF DISHWA” — a truncated part number is "
           "unsearchable, so the engine drops the sound level instead and reports what "
           "it dropped.", False, INK)],
     size=9, space_after=0, first=True, line=1.2)
note(s, "Both labelled rows reproduce exactly, 14 of 14 prose fields, from the same "
        "algorithm with no per-row tuning.")
