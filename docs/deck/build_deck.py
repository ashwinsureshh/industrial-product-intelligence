...
rich(tf, [("Unilog gave two fully worked delivery rows. ", True, NAVY),
          ("This is row 1, field by field — their value, then ours. "
           "Prose is generated by formula against their character limits, "
           "never by free text.", False, MUTED)],
...
rich(tf, [(f"{_sum['exact_given_attributes']}/{_sum['fields_scored']} exact when the "
          ...
          (" — because Series, Mounting, Wash Cycles and Voltage are not in that row. "
           "They are on frigidaire.com, which their own delivery row cites as MFR URL. "
           "The formatting is solved; the sourcing is the open problem, and that is "
           "what the 88-of-114 blank fields measure.", False, INK)],
     size=9, space_after=0, first=True, line=1.22)
note(s, "Do not quote 14/14 on its own. It says the formatter is exact given the "
        "values; it does not say the pipeline produces them from a catalogue row. "
        "Both numbers are on the slide for that reason.")
